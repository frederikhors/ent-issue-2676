...
func test(t *testing.T, client *ent.Client) {
	...
	client.Team.Delete().ExecX(ctx)

	teamOne := client.Team.Create().SetName("Team one").SaveX(ctx)

	client.User.Create().SetName("Ariel").SetAge(30).SetTeam(teamOne).ExecX(ctx)

	err := client.Team.DeleteOne(teamOne).Exec(ctx)
	if err != nil {
		if ent.IsConstraintError(err) {
			println("IsConstraintError is working!")
		} else {
			println("IsConstraintError is NOT working!")
		}
	}
}
